...
const { data: allTasks } = useQuery({
  ...
  queryFn: async () => {
    if (isAdmin) {
      ...
      // PM sees all tasks in their projects
      const { data, error } = await supabase
        .from('tasks')
        .select(`*, assignee:assigned_to (id, full_name), project:project_id (name)`)
        .eq('archived', false)
  ...
  // Employee with most open tasks — admin only
  const employeeTaskCounts = isAdmin
    ? allTasks?.reduce((acc: Record<string, { name: string; count: number }>, task) => {
  ...
  const topEmployee = Object.values(employeeTaskCounts ?? {}).sort((a, b) => b.count - a.count)[0];
  ...
  const filteredTasks = allTasks?.filter((t) => {
    ...
    return assigneeName.includes(search.toLowerCase()) ||
      taskTitle.includes(search.toLowerCase()) ||
      projectName.includes(search.toLowerCase());
  });

  const groupedTasks = filteredTasks?.reduce((acc: Record<string, { name: string; tasks: any[] }>, task) => {
    const assignee = task.assignee as any;
  ...
  return (
    <div style={{ padding: 32, color: '#FFFFFF' }}>
            ...
            {statCards.map((card) => (
              <div key={card.label} style={{ backgroundColor: '#111827', borderRadius: 14, padding: 20, border: '1px solid #1F2937' }}>
                ...
                <div style={{ fontSize: 32, fontWeight: 900, color: card.color, marginBottom: 4 }}>
                  {card.value}
                  {card.total !== null && <span style={{ fontSize: 16, color: '#374151', fontWeight: 400 }}>/{card.total}</span>}
                </div>
      ...
      {isPM && !isAdmin && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: 'Open', value: allTasks?.filter(t => t.status === 'open').length ?? 0, color: '#6B7280', icon: '📌' },
            { label: 'In Progress', value: allTasks?.filter(t => t.status === 'in_progress').length ?? 0, color: '#F97316', icon: '⚡' },
            { label: 'Done', value: allTasks?.filter(t => t.status === 'completed').length ?? 0, color: '#22C55E', icon: '✅' },
          ].map((card) => (
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {isAdmin && (
          <button
            ...
            style={{ padding: '10px 16px', backgroundColor: groupByEmployee ? '#F97316' : '#111827', border: '1px solid', borderColor: groupByEmployee ? '#F97316' : '#1F2937', borderRadius: 10, color: groupByEmployee ? '#0A0F1E' : '#9CA3AF', fontSize: 13, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            👤 Group by Employee
          </button>
        )}
      ...
      {/* Tasks — Grouped by Employee (admin only) */}
      {isAdmin && groupByEmployee ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <tbody>
                  {group.tasks.map((task, i) => (
                    <tr key={task.id} style={{ borderTop: i > 0 ? '1px solid #1F2937' : 'none' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: statusColors[task.status], backgroundColor: statusColors[task.status] + '20', padding: '3px 8px', borderRadius: 20 }}>{statusLabels[task.status]}</span>
                      </td>
                      {task.due_date && (
                        <td style={{ padding: '12px 16px', fontSize: 12, color: new Date(task.due_date) < new Date() ? '#EF4444' : '#6B7280' }}>
                          {new Date(task.due_date) < new Date() ? '⚠️ ' : '📅 '}{new Date(task.due_date).toLocaleDateString()}
                        </td>
                      )}
                    </tr>
                  ))}
          ...
          {sortedGroups.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: '#4B5563' }}>No tasks found.</div>}
        </div>
      ) : (
              ...
              {filteredTasks?.map((task, i) => {
                ...
                return (
                  <tr key={task.id} style={{ borderTop: '1px solid #1F2937', backgroundColor: i % 2 === 0 ? 'transparent' : '#0D132110' }}>
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ fontSize: 14, fontWeight: 600 }}>{task.title}</div>
                      {task.description && <div style={{ fontSize: 11, color: '#6B7280', marginTop: 2 }}>{task.description.slice(0, 50)}{task.description.length > 50 ? '...' : ''}</div>}
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: 13, color: '#9CA3AF' }}>{(task.project as any)?.name ?? 'Unknown'}</td>
                    {isAdmin && <td style={{ padding: '12px 16px', fontSize: 13, color: '#9CA3AF' }}>{(task.assignee as any)?.full_name ?? 'Unassigned'}</td>}
