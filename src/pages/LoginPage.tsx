    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0A0F1E', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ display: 'flex', backgroundColor: '#111827', borderRadius: 10, padding: 4, marginBottom: 16, border: '1px solid #1F2937' }}>
          {['login', 'signup'].map((m) => (
            <button key={m} onClick={() => { setMode(m); setError(''); setSuccess(''); }} style={{ flex: 1, padding: '10px', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: 13, letterSpacing: 1, backgroundColor: mode === m ? '#F97316' : 'transparent', color: mode === m ? '#0A0F1E' : '#6B7280' }}>
        ...
        <div style={{ backgroundColor: '#111827', borderRadius: 16, padding: 32, border: '1px solid #1F2937' }}>
          ...
          {mode === 'signup' && (
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Smith" style={{ width: '100%', padding: '12px 16px', backgroundColor: '#1F2937', border: '1px solid #374151', borderRadius: 10, color: '#FFFFFF', fontSize: 15, outline: 'none', boxSizing: 'border-box' }} />
            </div>
          )}
          ...
          {mode === 'signup' && (
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="text" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Smith Construction Co." style={{ width: '100%', padding: '12px 16px', backgroundColor: '#1F2937', border: '1px solid #374151', borderRadius: 10, color: '#FFFFFF', fontSize: 15, outline: 'none', boxSizing: 'border-box' }} />
            </div>
          )}
